...
def main() -> None:
    ...
    cells.append(code(
        ...
        "    pl_out = polars_player_stats().sort('total_won_usd', descending=True).head(20)\n"
        "    ctx['rows_out'] = pl_out.height\n\n"
        "with rss_guard('q1.duckdb'), bench('q1: top20 by total_won_usd', 'duckdb') as ctx:\n"
        "    db_out = con.sql('SELECT player, total_won_usd FROM player_stats ORDER BY total_won_usd DESC LIMIT 20').pl()\n"
    ...
    cells.append(md("## Query 2 — Top 20 by # bets won"))
    cells.append(code(
        "with rss_guard('q2.polars'), bench('q2: top20 by n_won', 'polars') as ctx:\n"
        "    pl_out = polars_player_stats().sort('n_won', descending=True).head(20)\n"
        "    ctx['rows_out'] = pl_out.height\n\n"
        "with rss_guard('q2.duckdb'), bench('q2: top20 by n_won', 'duckdb') as ctx:\n"
        "    db_out = con.sql('SELECT player, n_won FROM player_stats ORDER BY n_won DESC LIMIT 20').pl()\n"
        "    ctx['rows_out'] = db_out.height\n\n"
    ...
    cells.append(code(
        ...
        "              .sort('score', descending=True, nulls_last=True).head(20)\n"
    ...
    cells.append(code(
        ...
        "    pl_q1 = sub.sort('total_won_usd', descending=True).head(20)\n"
        "    pl_q2 = sub.sort('n_won',         descending=True).head(20)\n"
        "    pl_q3 = sub.filter(pl.col('win_rate').is_not_null()).sort('win_rate', descending=True).head(20)\n"
        ...
        "    db_q1 = con.sql(sub_sql + 'SELECT player, total_won_usd FROM sub ORDER BY total_won_usd DESC LIMIT 20').pl()\n"
        "    db_q2 = con.sql(sub_sql + 'SELECT player, n_won FROM sub ORDER BY n_won DESC LIMIT 20').pl()\n"
        "    db_q3 = con.sql(sub_sql + 'SELECT player, win_rate FROM sub WHERE win_rate IS NOT NULL ORDER BY win_rate DESC LIMIT 20').pl()\n"
        "    db_q4 = con.sql(sub_sql + 'SELECT player, win_rate * LN(GREATEST(total_won_usd, 1.0)) AS score '\n"
        "                              'FROM sub WHERE win_rate IS NOT NULL ORDER BY score DESC LIMIT 20').pl()\n"
